...
def forward_return_1m(date, stocks):
    # 前瞻 1M 收益（到下一个月末的收盘）
...
def assemble_training_set(asof_date):
    ...
    for d in mes[:-1]:  # 最后一个月末仅用于打标签，不作为起点横截面
        ...
        if X is None or X.empty:
            continue
        y = forward_return_1m(d, list(X.index))
        df = X.join(y, how='inner')
        df = df.dropna(subset=['fwd_1m'])
        if df.shape[0] < 20:
            continue
        X_list.append(df[g.features])
        y_list.append(df['fwd_1m'])
    if not X_list:
        return None, None
...
def train_update(context):
    ...
    # 月末训练：用截至今天的历史月末数据
    X, y = assemble_training_set(today)
    if X is None or X.empty:
        log.warn('训练数据为空，跳过模型更新。')
        return
    # 训练
    model = MLRegressor(random_state=42)
    model.fit(X.values, y.values)
    g.model = model
    log.info('[模型更新] 训练样本数: %d, 特征维度: %d, 使用: %s' % (len(y), X.shape[1], 'XGBoost' if _HAS_XGB else 'HistGB'))
...
def score_today(date):
    ...
    if X is None or X.empty:
        return pd.DataFrame()
    # 若模型缺失，临时训练一次（应急，防止第一次回测月初无模型）
    if g.model is None:
        Xtr, ytr = assemble_training_set(date)
        if Xtr is not None and not Xtr.empty:
            g.model = MLRegressor(random_state=42).fit(Xtr.values, ytr.values)
    if g.model is None:
        # 仍然没有模型，就用简单线性打分：动量-估值
        tmp = X.copy()
        simple = tmp[['ret_3m','ret_6m','ret_12m']].mean(axis=1) - tmp[['pe_ratio','pb_ratio','ps_ratio']].mean(axis=1)
        preds = simple.fillna(simple.median())
    else:
        preds = pd.Series(g.model.predict(X.values), index=X.index)
...
def rebalance(context):
    ...
    if sc is None or sc.empty:
        log.warn('无可交易标的，跳过。')
        return
    target_list = list(sc[sc['selected']].index)
    ...
    # 记录
    record(num=len(target_list), cash=cash / 1e6)


# ==================== 面试要点辅助：性能评估（可选） ====================
# JoinQuant 回测报告中会展示收益/回撤等，这里额外输出横截面 IC（月度）。
